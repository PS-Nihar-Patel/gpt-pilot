...
import { CircularProgress, TextField, Button, Grid, Typography, Paper, Snackbar, Alert, Chip } from '@mui/material';
...
const ConfigGenerator = () => {
  ...
  const handleSubmitVectorConfig = async () => {
    ...
    try {
      ...
      const apiEndpoint = `http://localhost:5147/api/S3Bucket/vector/${encodeURIComponent(s3BucketName)}`; // Adjust this endpoint as necessary
      const payload = { ...vectorConfig}; // Include s3BucketName in the payload
  ...
  const handleSubmitLlmConfig = async () => {
    ...
    try {
      ...
      const response = await axios.get(apiEndpoint); // Using GET as per task description, but this might be a POST in actual implementation depending on backend logic.
      if (response.status === 200) {
  ...
  return (
    <Grid container spacing={2} component={Paper} padding={2}>
      ...
      <Grid item xs={12} md={6}>
        ...
        />
        {['metadata_keys_to_include_in_embedding', 'metadata_keys_to_include_in_LLM_calls', 'metadata_schema'].map((field) => (
          <div key={field}>
            <TextField
              fullWidth
              margin="normal"
